...
<!--        show-summary-->
        ...
        <el-table-column label="分组下账号数量" min-width="120" prop="account_num">
          <template slot-scope="scope">
        ...
        <el-table-column label="任务开始时间" min-width="150" prop="itime" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ formatTimestamp(scope.row[scope.column.property]) }}
          </template>
        </el-table-column>
        <el-table-column label="任务结束时间" min-width="150" prop="end_time" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ formatTimestamp(scope.row[scope.column.property]) }}
          </template>
        </el-table-column>
    ...
    >
      <el-form ref="refAddModal" :model="addModal.formData" :rules="addModal.rules" label-width="120px" size="small">
        ...
        <el-form-item label="活跃时长" prop="active_minutes">
          <el-input v-model="addModal.formData.active_minutes" placeholder="请输入活跃时长" @input="changeInput" />
        </el-form-item>
        <el-form-item label="提交间隔" prop="interval">
          <el-input v-model="addModal.formData.interval" placeholder="请输入提交间隔" @input="changeInput" />
        </el-form-item>
    ...
    >
      <template v-if="detailModal.title ==='任务详情'">
        ...
        >
          ...
          <el-table-column label="已提交次数" min-width="130" prop="submit_num" sortable="custom" show-overflow-tooltip>
            <template slot-scope="scope">
              {{ scope.row[scope.column.property] ? scope.row[scope.column.property] : '0' }}
            </template>
          </el-table-column>
          <el-table-column label="上次提交时间" min-width="100" prop="last_time" show-overflow-tooltip>
            <template slot-scope="scope">
              {{ formatTimestamp(scope.row[scope.column.property]) }}
            </template>
          </el-table-column>
    ...
    return {
      ...
      addModal: {
        ...
        rules: {
          ...
          active_minutes: [{ required: true, message: '请输入活跃时长！', trigger: 'change' }],
          interval: [{ required: true, message: '请输入提交间隔！', trigger: 'change' }],
    ...
    addSubmit() {
      this.$refs.refAddModal.validate((v) => {
        if (v) {
          ...
          formData.active_minutes = formData.active_minutes ? Number(formData.active_minutes) : 0
          formData.interval = formData.interval ? Number(formData.interval) : 0
