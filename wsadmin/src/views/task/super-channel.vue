<template>
  <div>
      <el-form size="small" :inline="true" style="margin-top: 10px;">
        <el-form-item>
          <el-button type="primary" @click="jumpCreatTask(0,0)">{{ $t('sys_m062') }}</el-button>
        </el-form-item>
        <!-- <el-form-item>
        ...
        <!-- -->
        <el-form-item>
          <el-button type="danger" :disabled="checkIdArry.length==0" @click="handleGroupBtn(2)">{{ $t('sys_rai123')}}</el-button>
        </el-form-item> 
        <el-form-item>
      ...
      <div class="continer_main">
        <div class="tab_check_warp">
          ...
          <el-table :data="taskDataList" row-key="id" use-virtual border height="700" v-loading="loading" ref="serveTable" 
          element-loading-spinner="el-icon-loading" style="width: 100%;" @selection-change="handleSelectionChange" @row-click="rowSelectChange">  
            ...
            <el-table-column prop="ym_num" :label="$t('sys_rai128')" minWidth="130" />
            <el-table-column prop="zq_num" :label="$t('sys_rai095')" minWidth="160" />
            ...
            <el-table-column fixed="right" :label="$t('sys_c010')" width="280">
                <template slot-scope="scope">
...
import { groupsendmsg,getmarketgrouptasktwolist,dobatchdelmarketgrouptasktwo,dobatchstoppullgrouptask,dobatchclosepullgrouptask,getsysconfig,upsysconfig,doupautoadtwo,sendmessagetwo} from '@/api/task'
export default {
  ...
  methods: {
    showPropModel(){
      ...
      async handleUpdate(){
        this.isLoading =true;
        let {code} = await doupautoadtwo({id:this.blastForm.relpy_id,material_list:this.materialData});
      ...
      getPullTaskList(num){
        ...
        getmarketgrouptasktwolist(params).then(res=>{
          this.loading=false;
      ...
      goTaskDetail(row){
        this.$router.push({path:'/super-task-detail',query:{params:row,task_id:row.id}});
      },
      submitForm(formName){
        if(this.blastForm.explode_type == 2){
          this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              sendmessagetwo(params).then(res=>{
                this.isLoading=false;
          ...
          this.isLoading=true;
          sendmessagetwo({ids:this.blastForm.relpy_type==4?this.checkIdArry:[this.blastForm.relpy_id]}).then(res=>{
            this.isLoading=false;
            if (res.code !=0 ) return;
            successTips(this)
            this.getPullTaskList(1);
    ...
    beforeOneBtn(row){
      ...
      handleGroupBtn(type) {
          ...
          let that = this;
          console.log(type);
          let tipsText = type==1?that.$t('sys_rai081'):type==2?that.$t('sys_rai129'):type==3?that.$t('sys_rai097'):that.$t('sys_q111')
          that.$confirm(that.$t('sys_rai046',{value:tipsText}), that.$t('sys_l013'), {
            ...
            beforeClose: function (action, instance, done) {
                if (action === 'confirm') {
                  let reqApi = type==1?dobatchclosepullgrouptask:type==2?dobatchdelmarketgrouptasktwo:type==3?dobatchstoppullgrouptask:groupsendmsg;
                  instance.confirmButtonLoading = true;
